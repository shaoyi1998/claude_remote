...
### 核心功能
- **远程终端** - 基于 xterm.js 的完整终端模拟，实时 WebSocket 通信，支持触摸滚动
- **任务管理** - 创建、恢复、删除 Claude Code 任务，实时状态监控
- **快捷键系统** - 自定义命令按钮和快捷键组合，云端同步配置
- **文件管理** - 浏览目录、查看编辑文件、上传下载、图片/音频预览
- **反向代理** - 访问本地其他开发服务器（如 Vite、React 等）
- **用户系统** - JWT 认证、多用户支持、登录防爆破保护

### 侧边栏快捷面板
- **Home 模式 (H)** - 常用按键 + 自定义命令/快捷键
- **快捷键模式 (K)** - 显示所有自定义快捷键
- **命令模式 (C)** - 显示所有自定义命令
- **数字模式 (N)** - 方向键 + 0-9 数字键，方便输入
- **终端锁定** - 防止误触，支持触摸滚动查看历史

### 文件浏览器
- 目录浏览、面包屑导航
- Monaco Editor 代码编辑（支持语法高亮）
- 图片预览、音频播放
- 路径记忆（自动恢复上次浏览位置）
- 新建文件/文件夹、删除、下载
